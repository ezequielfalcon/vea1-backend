      ...
      jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
        if (err) {
          ...
          db.manyOrNone('SELECT productos.id, productos.nombre, productos.stock_minimo, productos.iva, ' +
                        'productos.codigo, categorias.nombre categoria, ' +
                        'unidades.nombre unidad, productos.es_ingrediente es_ingrediente ' +
                        'FROM productos ' +
      ...
      jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
        if (err) {
          ...
          if (req.params.id) {
            ...
            db.manyOrNone('SELECT id, nombre, stock_minimo, iva, codigo, id_categoria, id_unidad, id_marca, es_ingrediente ' +
                  ...
                  db.oneOrNone('SELECT id FROM productos WHERE codigo = $1 AND id_cliente_int = $2;'
                    ...
                    .then(codigoRepe => {
                      if (codigoRepe) {
                        ...
                        db.one('INSERT INTO productos (nombre, stock_minimo, iva, codigo, id_categoria, id_unidad, id_cliente_int, id_marca, es_ingrediente) ' +
                                                    'VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id;'
                          ,[req.body.nombre, stock_minimo, req.body.iva, codigo,
                          req.body.id_categoria, req.body.id_unidad, decoded.cliente, marca, req.body.es_ingrediente || false])
                          .then(nuevoP => {
