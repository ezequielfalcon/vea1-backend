              ...
              db.oneOrNone('SELECT id FROM remitos WHERE id = $1 AND id_cliente_int = $2;', [req.params.id, decoded.cliente])
                .then(remitoExiste => {
                  if (remitoExiste) {
                    ...
                    res.status(404).json({resultado: false, mensaje: 'El remito no existe!'})
                  }
                })
                .catch(err => {
                  console.error(err);
        ...
        if (err) {
          ...
          if (roles.includes('stock') || roles.includes('admin')) {
            if (req.body.id_remito  && req.body.id_producto && req.body.cantidad && req.body.costo) {
              db.oneOrNone('SELECT id FROM remitos WHERE id = $1 AND id_cliente_int = $2;', [req.body.id_remito, decoded.cliente])
                .then(remitoExiste => {
                  if (remitoExiste) {
          ...
          if (roles.includes('stock') || roles.includes('admin')) {
            if (req.params.id_remito) {
              db.manyOrNone('SELECT productos_por_remito.id_producto, productos_por_remito.cantidad, ' +
                ...
                'WHERE id_remito = $1 AND id_cliente_int = $2;', [req.params.id_remito, decoded.cliente])
                .then(productos => {
