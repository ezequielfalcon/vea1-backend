                ...
                if (err) {
                    ...
                    if ((roles.includes('stock') || roles.includes('admin'))) {
                        if (req.params.id && req.body.nombre && req.body.stock_minimo && req.body.codigo
                            ...
                            db.oneOrNone('SELECT nombre, id FROM productos WHERE codigo = $1 AND id_cliente_int = $2 LIMIT 1;', [req.body.codigo, decoded.cliente])
                                .then(codigoDb => {
                                    if (codigoDb && (req.params.id !== codigoDb.id)) {
